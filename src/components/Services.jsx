import React, { useRef, useState } from 'react';
import { motion, useScroll, useSpring, useMotionValueEvent } from 'framer-motion';
import { processContent } from '../data/portfolioData';

const TagCard = ({ number, title, text, className, aosDelay, aosType, pathLength, containerRef }) => {
  const ref = useRef(null);
  const [isActive, setIsActive] = useState(false);

  useMotionValueEvent(pathLength, "change", (latest) => {
    if (!ref.current || !containerRef.current) return;
    
    const cardRect = ref.current.getBoundingClientRect();
    const containerRect = containerRef.current.getBoundingClientRect();
    
    const cardTopRelativeToContainer = cardRect.top - containerRect.top;
    const containerHeight = containerRect.height;
    
    const triggerY = cardTopRelativeToContainer + 50;
    const lineTipY = latest * containerHeight;
    
    if (lineTipY >= triggerY && !isActive) {
      setIsActive(true);
    } else if (lineTipY < triggerY && isActive) {
      setIsActive(false);
    }
  });

  return (
    <div 
      ref={ref}
      data-aos={aosType || "fade-up"} 
      data-aos-delay={aosDelay}
      className={`w-72 sm:w-80 rounded-[2rem] p-2 relative flex flex-col items-center hover:scale-[1.02] transition-all duration-700 z-10 ${className} ${
        isActive 
          ? 'bg-[#111827] border border-slate-700 shadow-[0_20px_50px_rgba(17,24,39,0.3)]' 
          : 'glass-card border border-white/80 shadow-md'
      }`}
    >
      {/* Hole punch */}
      <div className="w-5 h-5 bg-gradient-to-br from-slate-300 to-slate-100 rounded-full shadow-[inset_0_2px_4px_rgba(0,0,0,0.3)] absolute top-4 border border-slate-300 z-10 flex items-center justify-center">
        <div className="w-2 h-2 bg-slate-800 rounded-full opacity-30"></div>
      </div>
      
      {/* Inner card container */}
      <div className={`w-full h-full rounded-[1.5rem] mt-8 p-7 flex flex-col min-h-[220px] transition-colors duration-700 ${
        isActive ? 'bg-slate-900/80 text-white' : 'bg-white/60 text-slate-900'
      }`}>
        <span className={`text-xl font-bold mb-2 font-mono transition-colors duration-700 ${
          isActive ? 'text-slate-300' : 'text-slate-400'
        }`}>{number}</span>
        
        <h3 className={`text-lg md:text-xl font-extrabold mb-3 tracking-tight transition-colors duration-700 ${
          isActive ? 'text-white' : 'text-[#111827]'
        }`}>{title}</h3>
        
        <p className={`text-xs md:text-sm leading-relaxed font-medium transition-colors duration-700 ${
          isActive ? 'text-slate-300' : 'text-slate-600'
        }`}>
          {text}
        </p>
      </div>
    </div>
  );
};

const Services = () => {
  const containerRef = useRef(null);
  
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start center", "end center"]
  });

  const pathLength = useSpring(scrollYProgress, { stiffness: 60, damping: 20, restDelta: 0.001 });

  return (
    <section 
      id="process"
      ref={containerRef}
      className="py-24 px-6 md:px-12 w-full relative overflow-hidden font-sans"
    >
      <div className="max-w-6xl mx-auto relative md:h-[1780px]">
        
        {/* Header Content */}
        <div data-aos="fade-up" className="md:absolute top-10 left-0 md:w-[450px] z-20 mb-16 md:mb-0">
          <div className="inline-block glass-pill px-5 py-1.5 text-xs text-slate-700 font-bold mb-6 shadow-sm">
            {processContent.badge}
          </div>
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-extrabold text-[#111827] leading-[1.1] mb-6 tracking-tight relative">
            {processContent.heading}
            {/* Hand-drawn arrow SVG */}
            <svg className="absolute -bottom-10 right-10 w-12 h-12 text-slate-800" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path d="M4 4 Q 10 10 15 15 M 15 15 L 10 15 M 15 15 L 15 10" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </h2>
          <p className="text-slate-600 text-sm md:text-base max-w-sm font-medium leading-relaxed">
            {processContent.description}
          </p>
        </div>

        {/* Desktop SVG Animated S-Curve Dashed Line (Extended to fit 5 cards and 1780px height) */}
        <svg 
          className="hidden md:block absolute top-0 left-0 w-full h-[1780px] pointer-events-none z-0" 
          viewBox="0 0 1000 1780" 
          preserveAspectRatio="none"
        >
          {/* Faint background path guide */}
          <path 
            d="M 650,120 C 450,220 200,320 300,520 C 400,720 750,670 700,890 C 650,1110 200,1060 300,1260 C 400,1460 750,1410 650,1600" 
            fill="none" 
            stroke="#cbd5e1" 
            strokeWidth="2" 
            strokeDasharray="8 10" 
          />

          {/* Mask to reveal the dashed path based on scroll */}
          <mask id="path-mask">
            <motion.path 
              d="M 650,120 C 450,220 200,320 300,520 C 400,720 750,670 700,890 C 650,1110 200,1060 300,1260 C 400,1460 750,1410 650,1600" 
              fill="none" 
              stroke="white" 
              strokeWidth="20" 
              style={{ pathLength }}
            />
          </mask>

          {/* The actual dark dashed line revealed as user scrolls */}
          <path 
            d="M 650,120 C 450,220 200,320 300,520 C 400,720 750,670 700,890 C 650,1110 200,1060 300,1260 C 400,1460 750,1410 650,1600" 
            fill="none" 
            stroke="#111827" 
            strokeWidth="2" 
            strokeDasharray="8 10" 
            mask="url(#path-mask)"
            className="drop-shadow-sm"
          />
        </svg>

        {/* Mobile Animated Vertical Dashed Line */}
        <svg 
          className="md:hidden absolute top-0 left-[50%] -translate-x-1/2 w-4 h-[100%] pointer-events-none z-0" 
          viewBox="0 0 4 100" 
          preserveAspectRatio="none"
        >
          <path 
            d="M 2,0 L 2,100" 
            fill="none" 
            stroke="#cbd5e1" 
            strokeWidth="4" 
            strokeDasharray="4 6" 
            vectorEffect="non-scaling-stroke"
          />
          <mask id="path-mask-mobile">
            <motion.path 
              d="M 2,0 L 2,100" 
              fill="none" 
              stroke="white" 
              strokeWidth="4" 
              style={{ pathLength }}
              vectorEffect="non-scaling-stroke"
            />
          </mask>
          <path 
            d="M 2,0 L 2,100" 
            fill="none" 
            stroke="#111827" 
            strokeWidth="4" 
            strokeDasharray="4 6" 
            mask="url(#path-mask-mobile)"
            vectorEffect="non-scaling-stroke"
          />
        </svg>

        {/* Cards Container */}
        <div className="flex flex-col gap-8 md:gap-12 items-center md:block relative z-10 w-full pt-4 md:pt-0 pb-12 md:pb-0">
          
          {processContent.cards.map((card, index) => {
            const positions = [
              "md:absolute md:top-[10px] md:right-[5%] lg:right-[10%] rotate-2 md:rotate-4",
              "md:absolute md:top-[380px] md:left-[5%] lg:left-[10%] -rotate-2 md:-rotate-4",
              "md:absolute md:top-[750px] md:right-[5%] lg:right-[10%] rotate-1 md:rotate-3",
              "md:absolute md:top-[1120px] md:left-[10%] lg:left-[15%] -rotate-1 md:-rotate-3",
              "md:absolute md:top-[1490px] md:right-[15%] lg:right-[20%] rotate-2 md:rotate-4",
            ];
            const aosTypes = ["fade-left", "fade-right", "fade-left", "fade-right", "fade-left"];
            const aosDelays = ["100", "200", "300", "400", "500"];

            return (
              <TagCard 
                key={card.number}
                number={card.number}
                title={card.title}
                text={card.text}
                className={positions[index]}
                aosType={aosTypes[index]}
                aosDelay={aosDelays[index]}
                pathLength={pathLength}
                containerRef={containerRef}
              />
            );
          })}

          {/* End Callout text */}
          <div 
            data-aos="fade-in" 
            data-aos-delay="700"
            className="hidden md:block absolute top-[1700px] left-[55%] font-cursive text-3xl text-slate-800 rotate-6"
          >
            {processContent.endText}
          </div>

        </div>

      </div>
    </section>
  );
};

export default Services;
